...
/// Core Poseidon hasher implementation without any external dependencies
...
mod tests {
	...
	#[test]
	fn test_empty_input() {
		let result = PoseidonCore::hash_padded(&[]);
	...
	fn test_single_byte() {
		let input = vec![42u8];
		let result = PoseidonCore::hash_padded(&input);
		assert_eq!(result.len(), 32);
	}
	...
	fn test_consistency() {
		let input = [4u8; 50];
		let iterations = 10;
		let current_hash = PoseidonCore::hash_padded(&input);

		for _ in 0..iterations {
			let hash1 = PoseidonCore::hash_padded(&current_hash);
			let hash2 = PoseidonCore::hash_padded(&current_hash);
	...
	fn test_different_inputs() {
		let input1 = [5u8; 32];
		let input2 = [6u8; 32];
		let hash1 = PoseidonCore::hash_padded(&input1);
		let hash2 = PoseidonCore::hash_padded(&input2);
	...
	fn test_poseidon_hash_input_sizes() {
		...
		for size in 1..=128 {
			...
			let hash = PoseidonCore::hash_padded(&input);
	...
	fn test_big_preimage() {
		for overflow in 1..=10 {
			...
			let _hash = PoseidonCore::hash_padded(&preimage.to_le_bytes());
		}
	}
	...
	fn test_circuit_preimage() {
		...
		let hash = PoseidonCore::hash_padded(&preimage.unwrap());
		let _hash = PoseidonCore::hash_padded(&hash);
	}
	...
	fn test_random_inputs() {
		...
		for hex_string in hex_strings.iter() {
			...
			let hash = PoseidonCore::hash_padded(&preimage);
			let _hash2 = PoseidonCore::hash_padded(&hash);
		}
	}
	...
	fn test_known_value_hashes() {
		...
		for (input, expected_hex) in vectors.iter() {
			let hash = PoseidonCore::hash_padded(input);
			assert_eq!(hex::encode(&hash), *expected_hex, "input: 0x{}", hex::encode(input));
		}
	}
	...
	fn test_hash_no_pad() {
		let input = b"test";
		let padded_hash = PoseidonCore::hash_padded(input);
		let no_pad_hash = PoseidonCore::hash_no_pad_bytes(input);
